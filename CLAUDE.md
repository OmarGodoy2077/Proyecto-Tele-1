...
| S-05 | La Planta de Chiquimula tiene una dotación menor (≈ 2–3 estaciones administrativas) y **ningún servidor de aplicaciones crítico local**; el ERP se consume desde Jutiapa por la VPN | El caso menciona "estaciones de trabajo administrativas" y un "cuarto de telecomunicaciones local" sin detallar número ni servidores | Fase 2 (§2.1.1); insumo del DRP (Fase 3) |
| S-06 | Existen equipos portátiles de personal de ventas y de gerencia, con el mismo perfil de configuración que las estaciones fijas | El caso no los menciona explícitamente pero describe personal de ventas y gerencia | Fase 2 (§2.1.2); controles de EDR, cifrado y DNS fuera de oficina |
| S-07 | El motor de base de datos del ERP corre sobre un SO dentro de la ventana de obsolescencia de D-10; su versión requiere evaluación de soporte | El caso no detalla el motor ni la versión del ERP | Fase 2 (§2.1.4, C-16); dimensionamiento en Fase 4 |
| S-08 | El servicio de directorio y autenticación interno es **Active Directory** | El caso menciona "dominio interno" y "consolas de administración de servidores" | Fase 2 (§2.2.3, §6.5); controles C-15, C-22, C-26 |
| S-09 | Los controles físicos del cuarto de telecomunicaciones de la Planta son limitados (equivalentes o inferiores a los del centro de datos de la Casa Matriz) | El caso no describe los controles físicos de la Planta | Fase 2 (§2.5, C-12); insumo del DRP |
